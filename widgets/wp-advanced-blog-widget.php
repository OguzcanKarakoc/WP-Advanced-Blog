...
class Wp_Advanced_Blog_Widget extends WP_Widget
{
        ...
        <div>
            ...
            <p>
                <label for="<?= $this->get_field_id($prefix . 'compare') ?>">
                    comment count<br/>
                    <small>Only show posts that fall under the criteria defined below</small>
                </label>
            </p>
            <div style="display: inline">
                <select style="width: 60%" name="<?= $this->get_field_name($prefix . 'compare') ?>"
                        id="<?= $this->get_field_id($prefix . 'compare') ?>">
                    <option value="=" <?php selected($instance[$prefix . 'compare'], '=') ?>>equals</option>
                    <option value="!=" <?php selected($instance[$prefix . 'compare'], '!=') ?>>Does not equal</option>
                    <option value=">" <?php selected($instance[$prefix . 'compare'], '>') ?>>Greater then</option>
                    <option value=">=" <?php selected($instance[$prefix . 'compare'], '>=') ?>>Greater or equal to</option>
                    <option value="<" <?php selected($instance[$prefix . 'compare'], '<') ?>>Smaller then</option>
                    <option value="<=" <?php selected($instance[$prefix . 'compare'], '<=') ?>>Smaller or equal to</option>
                </select>
                <input style="width: 37%;vertical-align: middle;" id="<?= $this->get_field_id($prefix . 'comment_count') ?>"
                       ...
                       type="number" value="<?= $instance[$prefix . 'comment_count'] ?>"/>
            </div>
    ...
    public function widget($args, $instance)
    {
        ...
        echo "<pre>";
        var_dump($instance[$prefix . 'cache_results']);
        echo "</pre>";
//        $arr = [
//            'title' => [
//                'id' => $this->get_field_id($prefix . 'title'),
...
//            'category__not_in' => $instance[$prefix . 'category__not_in'],
            'comment_count' => $instance[$prefix . 'comment_count'], // TODO:: Test this value - https://developer.wordpress.org/reference/classes/wp_query/
        ];
